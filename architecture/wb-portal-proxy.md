...
Кнопка выбора компании в шапке WB (`HeaderItemsBlock__profile-select`) **заменяется** статическим текстом с `displayName` кабинета MarketHacker (`account_label`). Модальное окно «Профиль» (переключение компаний, настройки, документы) скрывается и блокируется. Менеджер не может сменить юрлицо или выйти из сессии WB.
