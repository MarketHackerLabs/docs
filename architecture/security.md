...
```python
class MarketplaceCredentialVault:
    encrypted_payload: bytes  # ciphertext (authorizev3, cookies, local_storage)
...
Сервер добавляет `SERVER_ONLY_COOKIE_KEYS` к каждому исходящему запросу к WB. Браузер менеджера не может прочитать или подменить эти значения. Подробнее: [WB Gateway & Guided Connect](./wb-portal-proxy.md#set-cookie-relay).
...
- Сброс пароля: одноразовая ссылка, срок жизни, без повторного использования;
  после смены — выход со всех устройств. Из админки — только письмо со ссылкой
  (право `users:password:reset` у сотрудников поддержки).
- Секрет MFA в БД шифруется; резервные коды хранятся только как хеши;
  сброс MFA — отдельное право `users:mfa:reset`.
