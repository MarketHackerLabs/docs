...
3. Если `useReviews` — подтянуть последний completed `reviews_analysis` по этому nm (или короткий сэмпл отзывов, если анализа нет).
4. LLM → JSON-отчёт (оценка, риски P0/P1, воронка слайдов, промт).
