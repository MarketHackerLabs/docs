...
|------------|-----------------|-----|
| RBAC permissions | `perms:{user_id}:{org_id}` | = JWT access TTL |
| Proxy sessions | `proxy:{token}` | 1 ч, sliding |
| Portal callback / capture | `portal_callback:*`, `portal_capture:*`, `portal_capture_cookies:*` | 5 мин / 30 мин |
...
Подробнее: [Аутентификация](./authentication.md), [WB Portal Proxy](./wb-portal-proxy.md).
