...
Смерть cookie алертится (`StorefrontCookieDead`, см. мониторинг в разделе Ozon).
...
### Мониторинг «смерти» cookie

Раз в 15 минут ARQ cron (`probe_storefront_sessions`) гоняет ту же пробу, что кнопка
«Проверить» в admin. `probe_ok=false` ставится только при антиботе витрины, не при
сбое прокси или транспорта. Тот же флаг выставляет адаптер при исчерпании antibot-ретраев
во время анализа отзывов.

Метрика backend `/metrics` (`reviews_analysis` collector):

| Значение `mh_storefront_cookie_ok{marketplace}` | Смысл |
|--------------------------------------------------|--------|
| `1` | последняя проба успешна |
| `0` | cookie мертва (антибот) |
| series нет | сессия не задана или ещё не проверялась |

Алерт Grafana `StorefrontCookieDead` (critical, 5 мин) уходит в Telegram через
Alertmanager. После обновления cookie и успешной пробы приходит RESOLVED.
